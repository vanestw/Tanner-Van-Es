	...
	do {
		...
		switch(choice) {
			case 1:
				...
				break; // ends case 1
			case 2:
				cerr << "NEED TO WRITE THIS METHOD" << endl;
				break;
			case 3:
				throw runtime_error("Goodbye!");
...
void Dealership::PrintEntirCustomerVector() const {
...
Vehicle* Dealership::FindVehicleObj(const string& vehicleToFind) const {
...
// Method to allow user to filter what they see when they view vehicle information
void Dealership::FilterVehiclePrintResults(const string& cRole) const { 
...
void Dealership::PrintEntireSalesVector() const {
	...
	while(getline(inputFile, line)) {
		if(line.find("Sale") != string::npos) {
			...
			if(!salesmanName.empty() && !salePriceStr.empty() && !salesByDayStr.empty() && !salesByWeekStr.empty() && !salesByMonthStr.empty() && !customerName.empty() && !vehicleVin.empty()) {
				...
				Vehicle* vehicle;
				vehicle = FindVehicleObj(vehicleVin);
